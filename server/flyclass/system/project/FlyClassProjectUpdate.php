...
/**------------------------------------*
 * 作者：shark
 * 创建时间：2020-10-04 17:40:48
 * Fly编码：1601804448129FLY078416
 * 类对象名：ObjFlyProjectUpdate()
 * ------------------------------------ */

//引入区

class FlyClassProjectUpdate{
    
    
    //---------- 类成员（member） ----------
    
    //类描述
    public static $classDescript = "项目维护";
    //类数据表名
    public static $tableName = "fly_project_update";
    ...
    /**
     * 函数名称：项目维护:游客:记录查询
     * 函数调用：ObjFlyProjectUpdate() -> VisitorFlyProjectUpdatePaging()
     * 创建时间：2020-10-04 17:40:48
     * */
    public function VisitorFlyProjectUpdatePaging(){
        ...
        //$vDataField = "id,onlyKey,descript,indexNumber,updateTime,addTime,shelfState,state,adminId,updateId,updateType,updateTitle,updateBody";
    ...
    /**
     * 函数名称：项目维护:系统:记录添加
     * 函数调用：ObjFlyProjectUpdate() -> SystemFlyProjectUpdateAdd
     * 创建时间：2020-10-04 17:40:47
     * */
    public function SystemFlyProjectUpdateAdd($fpAdminId,$fpUpdateId,$fpUpdateType,$fpUpdateTitle,$fpUpdateBody){
        
        //--- Json组合区 ---
        //Json数组
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "descript,shelfState,adminId,updateId,updateType,updateTitle,updateBody",
            "descript" => self::$classDescript,
            "shelfstate" => "false",
            "adminid" => $fpAdminId,
            "updateid" => $fpUpdateId,
            "updatetype" => $fpUpdateType,
            "updatetitle" => $fpUpdateTitle,
            "updatebody" => $fpUpdateBody,
            "key_field" => "adminId,updateId",
        );
        //执行:添加
    ...
    /**
     * 函数名称：项目维护:用户:记录查询
     * 函数调用：ObjFlyProjectUpdate() -> UserFlyProjectUpdatePaging($fpUserId)
     * 创建时间：2020-10-04 17:40:48
     * */
    public function UserFlyProjectUpdatePaging($fpUserId){
    ...
    /**
     * 函数名称：项目维护:管理员:记录添加
     * 函数调用：ObjFlyProjectUpdate() -> AdminFlyProjectUpdateAdd($fpAdminId)
     * 创建时间：2020-10-04 17:40:47
     * */
    public function AdminFlyProjectUpdateAdd($fpAdminId){
        ...
        //参数:维护ID:updateId
        $pUpdateId = GetParameterNoCode("updateid",$json);
        if(!JudgeRegularInt($pUpdateId)){return JsonModelParameterException("updateid", $pUpdateId, 11, "值必须是整数", __LINE__);}
        
        //参数:维护类型:updateType
        $pUpdateType = GetParameterNoCode("updatetype",$json);
        if(!JudgeRegularFont($pUpdateType)){return JsonModelParameterException("updatetype", $pUpdateType, 36, "内容格式错误", __LINE__);}
        
        //参数:维护标题:updateTitle
        $pUpdateTitle = GetParameterNoCode("updatetitle",$json);
        if(!JudgeRegularFont($pUpdateTitle)){return JsonModelParameterException("updatetitle", $pUpdateTitle, 64, "内容格式错误", __LINE__);}
        
        //参数:维护内容:updateBody
        $pUpdateBody = GetParameterNoCode("updatebody",$json);
        if(JudgeRegularComma($pUpdateBody)){return JsonModelParameterException("updatebody", $pUpdateBody, 512, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = array(
            ...
            "updatetitle" => $pUpdateTitle,
            "updatebody" => $pUpdateBody,
        );
        //执行:添加
        $vJsonResult = MIndexDataInsert(JsonHandleArray($jsonKeyValueArray));
    ...
    /**
     * 函数名称：项目维护:管理员:记录查询
     * 函数调用：ObjFlyProjectUpdate() -> AdminFlyProjectUpdatePaging($fpAdminId)
     * 创建时间：2020-10-04 17:40:48
     * */
    public function AdminFlyProjectUpdatePaging($fpAdminId){
        ...
        //参数:维护ID:updateId
        $pUpdateId = GetParameterNoCode("updateid",$json);
        if(!IsNull($pUpdateId)&&!JudgeRegularInt($pUpdateId)){return JsonModelParameterException("updateid", $pUpdateId, 11, "值必须是整数", __LINE__);}
        ...
        //条件字段判断组合
        $jsonKeyValueArray = HandleFlyJsonAddWhereField($jsonKeyValueArray, "id", $pId);
        $jsonKeyValueArray = HandleFlyJsonAddWhereField($jsonKeyValueArray, "updateId", $pUpdateId);
        $jsonKeyValueArray = HandleFlyJsonAddWhereField($jsonKeyValueArray, "state", $pWhereState);
        $jsonKeyValueArray = HandleFlyJsonAddWhereField($jsonKeyValueArray, "shelfState", $pWhereShelfState);
        $jsonKeyValueArray = HandleFlyJsonAddWhereField($jsonKeyValueArray, $pStateField, $pStateKey);
        //返回结果:分页
    ...
    /**
     * 函数名称：项目维护:管理员:记录修改
     * 函数调用：ObjFlyProjectUpdate() -> AdminFlyProjectUpdateSet($fpAdminId)
     * 创建时间：2020-10-04 17:40:47
     * */
    public function AdminFlyProjectUpdateSet($fpAdminId){
        ...
        //参数:维护类型:updateType
        $pUpdateType = GetParameterNoCode("updatetype",$json);
        if(!IsNull($pUpdateType)&&!JudgeRegularFont($pUpdateType)){return JsonModelParameterException("updatetype", $pUpdateType, 36, "内容格式错误", __LINE__);}
        
        //参数:维护标题:updateTitle
        $pUpdateTitle = GetParameterNoCode("updatetitle",$json);
        if(!IsNull($pUpdateTitle)&&!JudgeRegularFont($pUpdateTitle)){return JsonModelParameterException("updatetitle", $pUpdateTitle, 64, "内容格式错误", __LINE__);}
        
        //参数:维护内容:updateBody
        $pUpdateBody = GetParameterNoCode("updatebody",$json);
        if(!IsNull($pUpdateBody)&&!JudgeRegularFont($pUpdateBody)){return JsonModelParameterException("updatebody", $pUpdateBody, 512, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "updateId", $pUpdateId);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "updateType", $pUpdateType);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "updateTitle", $pUpdateTitle);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "updateBody", $pUpdateBody);
        
        //判断字段值是否为空
        $vUpdateFieldJudgeInfor = JudgeFlyJsonAddUpdateField($jsonKeyValueArray,"descript,indexnumber,shelfstate,state,adminid,updateid,updatetype,updatetitle,updatebody");
    ...
    /**
     * 函数名称：项目维护:管理员:记录状态修改
     * 函数调用：ObjFlyProjectUpdate() -> AdminFlyProjectUpdateSetState($fpAdminId)
     * 创建时间：2020-10-04 17:40:48
     * */
    public function AdminFlyProjectUpdateSetState($fpAdminId){
    ...
    /**
     * 函数名称：项目维护:管理员:数据上下架状态修改
     * 函数调用：ObjFlyProjectUpdate() -> AdminFlyProjectUpdateShelfState($fpAdminId)
     * 创建时间：2020-10-04 17:40:48
     * */
    public function AdminFlyProjectUpdateShelfState($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：项目维护:管理员:记录永久删除
     * 函数调用：ObjFlyProjectUpdate() -> AdminFlyProjectUpdateDelete($fpAdminId)
     * 创建时间：2020-10-04 17:40:48
     * */
    public function AdminFlyProjectUpdateDelete($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：获取数据表名称
     * 函数调用：ObjFlyProjectUpdate() -> GetTableName()
     * 创建时间：2020-10-04 17:40:47
     * */
    ...
    /**
     * 函数名称：获取类描述
     * 函数调用：ObjFlyProjectUpdate() -> GetClassDescript()
     * 创建时间：2020-10-04 17:40:47
     * */
    ...
    /**
     * 函数名称：获取数据表字段
     * 函数调用：ObjFlyProjectUpdate() -> GetTableField()
     * 创建时间：2020-10-04 17:40:47
     * */
    ...
    /**
     * 函数名称：类数据表基础字段检测
     * 函数调用：ObjFlyProjectUpdate() -> OprationTableFieldBaseCheck()
     * 创建时间：2020-10-04 17:40:47
     * */
