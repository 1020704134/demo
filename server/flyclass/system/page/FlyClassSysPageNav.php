...
/**------------------------------------*
 * 作者：shark
 * 创建时间：2020-10-15 17:15:12
 * Fly编码：1602753312398FLY445306
 * 类对象名：ObjSysPageNav()
 * ------------------------------------ */

//引入区

class FlyClassSysPageNav{
    
    
    //---------- 类成员（member） ----------
    
    //类描述
    public static $classDescript = "系统-页面-导航";
    //类数据表名
    public static $tableName = "sys_page_nav";
    ...
    /**
     * 函数名称：系统-页面-导航:游客:记录查询
     * 函数调用：ObjSysPageNav() -> VisitorSysPageNavPaging()
     * 创建时间：2020-10-15 17:15:12
     * */
    public function VisitorSysPageNavPaging(){
        ...
        //$vDataField = "id,onlyKey,descript,indexNumber,updateTime,addTime,shelfState,state,recodeName,recodeGroup,recodeUpId,navIcon,navName,navHref";
    ...
    /**
     * 函数名称：系统-页面-导航:系统:记录添加
     * 函数调用：ObjSysPageNav() -> SystemSysPageNavAdd
     * 创建时间：2020-10-15 17:15:12
     * */
    public function SystemSysPageNavAdd($fpRecodeName,$fpRecodeGroup,$fpRecodeUpId,$fpNavIcon,$fpNavName,$fpNavHref){
        
        //--- Json组合区 ---
        //Json数组
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "recodeName,recodeGroup,recodeUpId,navIcon,navName,navHref",
    ...
    /**
     * 函数名称：系统-页面-导航:用户:记录查询
     * 函数调用：ObjSysPageNav() -> UserSysPageNavPaging($fpUserId)
     * 创建时间：2020-10-15 17:15:12
     * */
    public function UserSysPageNavPaging($fpUserId){
    ...
    /**
     * 函数名称：系统-页面-导航:管理员:记录添加
     * 函数调用：ObjSysPageNav() -> AdminSysPageNavAdd($fpAdminId)
     * 创建时间：2020-10-15 17:15:12
     * */
    public function AdminSysPageNavAdd($fpAdminId){
        ...
        //参数:导航图标图片:navIcon
        $pNavIcon = GetParameterNoCode("navicon",$json);
        if(!JudgeRegularUrl($pNavIcon)){return JsonModelParameterException("navicon", $pNavIcon, 128, "URL地址格式错误", __LINE__);}
        
        //参数:导航名称:navName
        $pNavName = GetParameterNoCode("navname",$json);
        if(!JudgeRegularFont($pNavName)){return JsonModelParameterException("navname", $pNavName, 36, "内容格式错误", __LINE__);}
        
        //参数:导航链接:navHref
        $pNavHref = GetParameterNoCode("navhref",$json);
        if(!JudgeRegularUrl($pNavHref)){return JsonModelParameterException("navhref", $pNavHref, 128, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "navIcon", $pNavIcon);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "navName", $pNavName);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "navHref", $pNavHref);
    ...
    /**
     * 函数名称：系统-页面-导航:管理员:记录查询
     * 函数调用：ObjSysPageNav() -> AdminSysPageNavPaging($fpAdminId)
     * 创建时间：2020-10-15 17:15:12
     * */
    public function AdminSysPageNavPaging($fpAdminId){
    ...
    /**
     * 函数名称：系统-页面-导航:管理员:记录修改
     * 函数调用：ObjSysPageNav() -> AdminSysPageNavSet($fpAdminId)
     * 创建时间：2020-10-15 17:15:12
     * */
    public function AdminSysPageNavSet($fpAdminId){
        ...
        //参数:导航图标图片:navIcon
        $pNavIcon = GetParameterNoCode("navicon",$json);
        if(!IsNull($pNavIcon)&&!JudgeRegularUrl($pNavIcon)){return JsonModelParameterException("navicon", $pNavIcon, 128, "URL地址格式错误", __LINE__);}
        
        //参数:导航名称:navName
        $pNavName = GetParameterNoCode("navname",$json);
        if(!IsNull($pNavName)&&!JudgeRegularFont($pNavName)){return JsonModelParameterException("navname", $pNavName, 36, "内容格式错误", __LINE__);}
        
        //参数:导航链接:navHref
        $pNavHref = GetParameterNoCode("navhref",$json);
        if(!IsNull($pNavHref)&&!JudgeRegularFont($pNavHref)){return JsonModelParameterException("navhref", $pNavHref, 128, "内容格式错误", __LINE__);}
        ...
        //判断字段值是否为空
        $vUpdateFieldJudgeInfor = JudgeFlyJsonAddUpdateField($jsonKeyValueArray,"descript,indexnumber,shelfstate,state,recodename,recodegroup,recodeupid,navicon,navname,navhref");
    ...
    /**
     * 函数名称：系统-页面-导航:管理员:记录状态修改
     * 函数调用：ObjSysPageNav() -> AdminSysPageNavSetState($fpAdminId)
     * 创建时间：2020-10-15 17:15:12
     * */
    public function AdminSysPageNavSetState($fpAdminId){
    ...
    /**
     * 函数名称：系统-页面-导航:管理员:数据上下架状态修改
     * 函数调用：ObjSysPageNav() -> AdminSysPageNavShelfState($fpAdminId)
     * 创建时间：2020-10-15 17:15:12
     * */
    public function AdminSysPageNavShelfState($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：系统-页面-导航:管理员:记录永久删除
     * 函数调用：ObjSysPageNav() -> AdminSysPageNavDelete($fpAdminId)
     * 创建时间：2020-10-15 17:15:12
     * */
    public function AdminSysPageNavDelete($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：获取数据表名称
     * 函数调用：ObjSysPageNav() -> GetTableName()
     * 创建时间：2020-10-15 17:15:12
     * */
    ...
    /**
     * 函数名称：获取类描述
     * 函数调用：ObjSysPageNav() -> GetClassDescript()
     * 创建时间：2020-10-15 17:15:12
     * */
    ...
    /**
     * 函数名称：获取数据表字段
     * 函数调用：ObjSysPageNav() -> GetTableField()
     * 创建时间：2020-10-15 17:15:12
     * */
    ...
    /**
     * 函数名称：类数据表基础字段检测
     * 函数调用：ObjSysPageNav() -> OprationTableFieldBaseCheck()
     * 创建时间：2020-10-15 17:15:12
     * */
