...
/**------------------------------------*
 * 作者：shark
 * 创建时间：2020-10-15 20:39:20
 * Fly编码：1602765560792FLY174339
 * 类对象名：ObjFlyOptionMenuSon()
 * ------------------------------------ */

//引入区

class FlyClassOptionMenuSon{
    
    
    //---------- 类成员（member） ----------
    
    //类描述
    public static $classDescript = "Fly-操作手册-菜单子项";
    //类数据表名
    public static $tableName = "fly_option_menu_son";
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:游客:记录查询
     * 函数调用：ObjFlyOptionMenuSon() -> VisitorFlyOptionMenuSonPaging()
     * 创建时间：2020-10-15 20:39:20
     * */
    public function VisitorFlyOptionMenuSonPaging(){
        ...
        //$vDataField = "id,onlyKey,descript,indexNumber,updateTime,addTime,shelfState,state,menuId,menuSonName,menuSonDescript";
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:系统:记录添加
     * 函数调用：ObjFlyOptionMenuSon() -> SystemFlyOptionMenuSonAdd
     * 创建时间：2020-10-15 20:39:20
     * */
    public function SystemFlyOptionMenuSonAdd($fpMenuId,$fpMenuSonName,$fpMenuSonDescript){
        
        //--- Json组合区 ---
        //Json数组
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "menuId,menuSonName,menuSonDescript",
            "menuid" => $fpMenuId,
            "menusonname" => $fpMenuSonName,
            "menusondescript" => $fpMenuSonDescript,
            //"key_field" => "menuId",
        );
        //执行:添加
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:用户:记录查询
     * 函数调用：ObjFlyOptionMenuSon() -> UserFlyOptionMenuSonPaging($fpUserId)
     * 创建时间：2020-10-15 20:39:20
     * */
    public function UserFlyOptionMenuSonPaging($fpUserId){
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:管理员:记录添加
     * 函数调用：ObjFlyOptionMenuSon() -> AdminFlyOptionMenuSonAdd($fpAdminId)
     * 创建时间：2020-10-15 20:39:20
     * */
    public function AdminFlyOptionMenuSonAdd($fpAdminId){
        ...
        //参数:菜单ID:menuId
        $pMenuId = GetParameterNoCode("menuid",$json);
        if(!JudgeRegularInt($pMenuId)){return JsonModelParameterException("menuid", $pMenuId, 11, "值必须是整数", __LINE__);}
        
        //参数:菜单子项名称:menuSonName
        $pMenuSonName = GetParameterNoCode("menusonname",$json);
        if(!JudgeRegularFont($pMenuSonName)){return JsonModelParameterException("menusonname", $pMenuSonName, 36, "内容格式错误", __LINE__);}
        
        //参数:菜单子项描述:menuSonDescript
        $pMenuSonDescript = GetParameterNoCode("menusondescript",$json);
        if(!JudgeRegularFont($pMenuSonDescript)){return JsonModelParameterException("menusondescript", $pMenuSonDescript, 256, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = array(
            ...
            "insert_field" => "menuId,menuSonName,menuSonDescript",
            "menuid" => $pMenuId,
            "menusonname" => $pMenuSonName,
            "menusondescript" => $pMenuSonDescript,
            //"key_field" => "menuId",
            //- 修改记录 -
            "where_field" => "",
            "where_value" => "",
        ...
        //修改字段判断组合
        $jsonKeyValueArray = HandleFlyJsonAddWhereField($jsonKeyValueArray, "id", $pId);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "menuId", $pMenuId);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "menuSonName", $pMenuSonName);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "menuSonDescript", $pMenuSonDescript);
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:管理员:记录查询
     * 函数调用：ObjFlyOptionMenuSon() -> AdminFlyOptionMenuSonPaging($fpAdminId)
     * 创建时间：2020-10-15 20:39:20
     * */
    public function AdminFlyOptionMenuSonPaging($fpAdminId){
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:管理员:记录修改
     * 函数调用：ObjFlyOptionMenuSon() -> AdminFlyOptionMenuSonSet($fpAdminId)
     * 创建时间：2020-10-15 20:39:20
     * */
    public function AdminFlyOptionMenuSonSet($fpAdminId){
        ...
        //参数:菜单ID:menuId
        $pMenuId = GetParameterNoCode("menuid",$json);
        if(!IsNull($pMenuId)&&!JudgeRegularInt($pMenuId)){return JsonModelParameterException("menuid", $pMenuId, 11, "值必须是整数", __LINE__);}
        
        //参数:菜单子项名称:menuSonName
        $pMenuSonName = GetParameterNoCode("menusonname",$json);
        if(!IsNull($pMenuSonName)&&!JudgeRegularFont($pMenuSonName)){return JsonModelParameterException("menusonname", $pMenuSonName, 36, "内容格式错误", __LINE__);}
        
        //参数:菜单子项描述:menuSonDescript
        $pMenuSonDescript = GetParameterNoCode("menusondescript",$json);
        if(!IsNull($pMenuSonDescript)&&!JudgeRegularFont($pMenuSonDescript)){return JsonModelParameterException("menusondescript", $pMenuSonDescript, 256, "内容格式错误", __LINE__);}
        ...
        //判断字段值是否为空
        $vUpdateFieldJudgeInfor = JudgeFlyJsonAddUpdateField($jsonKeyValueArray,"descript,indexnumber,shelfstate,state,menuid,menusonname,menusondescript");
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:管理员:记录状态修改
     * 函数调用：ObjFlyOptionMenuSon() -> AdminFlyOptionMenuSonSetState($fpAdminId)
     * 创建时间：2020-10-15 20:39:20
     * */
    public function AdminFlyOptionMenuSonSetState($fpAdminId){
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:管理员:数据上下架状态修改
     * 函数调用：ObjFlyOptionMenuSon() -> AdminFlyOptionMenuSonShelfState($fpAdminId)
     * 创建时间：2020-10-15 20:39:20
     * */
    public function AdminFlyOptionMenuSonShelfState($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：Fly-操作手册-菜单子项:管理员:记录永久删除
     * 函数调用：ObjFlyOptionMenuSon() -> AdminFlyOptionMenuSonDelete($fpAdminId)
     * 创建时间：2020-10-15 20:39:20
     * */
    public function AdminFlyOptionMenuSonDelete($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：获取数据表名称
     * 函数调用：ObjFlyOptionMenuSon() -> GetTableName()
     * 创建时间：2020-10-15 20:39:20
     * */
    ...
    /**
     * 函数名称：获取类描述
     * 函数调用：ObjFlyOptionMenuSon() -> GetClassDescript()
     * 创建时间：2020-10-15 20:39:20
     * */
    ...
    /**
     * 函数名称：获取数据表字段
     * 函数调用：ObjFlyOptionMenuSon() -> GetTableField()
     * 创建时间：2020-10-15 20:39:20
     * */
    ...
    /**
     * 函数名称：类数据表基础字段检测
     * 函数调用：ObjFlyOptionMenuSon() -> OprationTableFieldBaseCheck()
     * 创建时间：2020-10-15 20:39:20
     * */
