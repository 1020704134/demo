...
/**------------------------------------*
 * 作者：shark
 * 创建时间：2020-10-05 09:29:28
 * Fly编码：1601861368043FLY013064
 * 类对象名：ObjFlyProjectTimeaxisTitle()
 * ------------------------------------ */

//引入区

class FlyClassProjectTimeaxisTitle{
    
    
    //---------- 类成员（member） ----------
    
    //类描述
    public static $classDescript = "项目时间轴标题";
    //类数据表名
    public static $tableName = "fly_project_timeaxis_title";
    ...
    /**
     * 函数名称：项目时间轴标题:游客:记录查询
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> VisitorFlyProjectTimeaxisTitlePaging()
     * 创建时间：2020-10-05 09:29:27
     * */
    public function VisitorFlyProjectTimeaxisTitlePaging(){
        ...
        //$vDataField = "id,onlyKey,descript,indexNumber,updateTime,addTime,shelfState,state,versionId,versionTitle";
    ...
    /**
     * 函数名称：项目时间轴标题:系统:记录添加
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> SystemFlyProjectTimeaxisTitleAdd
     * 创建时间：2020-10-05 09:29:27
     * */
    public function SystemFlyProjectTimeaxisTitleAdd($fpVersionId,$fpVersionTitle){
        
        //--- Json组合区 ---
        //Json数组
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "descript,shelfState,versionId,versionTitle",
            "descript" => self::$classDescript,
            "shelfstate" => "false",
            "versionid" => $fpVersionId,
            "versiontitle" => $fpVersionTitle,
            "key_field" => "versionId",
        );
        //执行:添加
    ...
    /**
     * 函数名称：项目时间轴标题:用户:记录查询
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> UserFlyProjectTimeaxisTitlePaging($fpUserId)
     * 创建时间：2020-10-05 09:29:27
     * */
    public function UserFlyProjectTimeaxisTitlePaging($fpUserId){
    ...
    /**
     * 函数名称：项目时间轴标题:管理员:记录添加
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> AdminFlyProjectTimeaxisTitleAdd($fpAdminId)
     * 创建时间：2020-10-05 09:29:27
     * */
    public function AdminFlyProjectTimeaxisTitleAdd($fpAdminId){
        ...
        //参数:版本ID:versionId
        $pVersionId = GetParameterNoCode("versionid",$json);
        if(!JudgeRegularFont($pVersionId)){return JsonModelParameterException("versionid", $pVersionId, 20, "内容格式错误", __LINE__);}
        
        //参数:版本标题:versionTitle
        $pVersionTitle = GetParameterNoCode("versiontitle",$json);
        if(!JudgeRegularFont($pVersionTitle)){return JsonModelParameterException("versiontitle", $pVersionTitle, 64, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = array(
            ...
            "versiontitle" => $pVersionTitle,
            "key_field" => "versionId,versionTitle",
        );
        //执行:添加
        $vJsonResult = MIndexDataInsert(JsonHandleArray($jsonKeyValueArray));
    ...
    /**
     * 函数名称：项目时间轴标题:管理员:记录查询
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> AdminFlyProjectTimeaxisTitlePaging($fpAdminId)
     * 创建时间：2020-10-05 09:29:28
     * */
    public function AdminFlyProjectTimeaxisTitlePaging($fpAdminId){
        ...
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "data_field" => $vDataField,
            "where_field" => "versionId",
            "where_value" => "{$pVersionId}",
            "page" => $pPage,
            "limit" => $pLimit,
            //"descbo" => "true",
            //"orderby" => "id",
    ...
    /**
     * 函数名称：项目时间轴标题:管理员:记录修改
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> AdminFlyProjectTimeaxisTitleSet($fpAdminId)
     * 创建时间：2020-10-05 09:29:27
     * */
    public function AdminFlyProjectTimeaxisTitleSet($fpAdminId){
        ...
        //参数:版本ID:versionId
        $pVersionId = GetParameterNoCode("versionid",$json);
        if(!IsNull($pVersionId)&&!JudgeRegularFont($pVersionId)){return JsonModelParameterException("versionid", $pVersionId, 20, "内容格式错误", __LINE__);}
        
        //参数:版本标题:versionTitle
        $pVersionTitle = GetParameterNoCode("versiontitle",$json);
        if(!IsNull($pVersionTitle)&&!JudgeRegularFont($pVersionTitle)){return JsonModelParameterException("versiontitle", $pVersionTitle, 64, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "versionId", $pVersionId);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "versionTitle", $pVersionTitle);
        
        //判断字段值是否为空
        $vUpdateFieldJudgeInfor = JudgeFlyJsonAddUpdateField($jsonKeyValueArray,"descript,indexnumber,shelfstate,state,versionid,versiontitle");
    ...
    /**
     * 函数名称：项目时间轴标题:管理员:记录状态修改
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> AdminFlyProjectTimeaxisTitleSetState($fpAdminId)
     * 创建时间：2020-10-05 09:29:27
     * */
    public function AdminFlyProjectTimeaxisTitleSetState($fpAdminId){
    ...
    /**
     * 函数名称：项目时间轴标题:管理员:数据上下架状态修改
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> AdminFlyProjectTimeaxisTitleShelfState($fpAdminId)
     * 创建时间：2020-10-05 09:29:27
     * */
    public function AdminFlyProjectTimeaxisTitleShelfState($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：项目时间轴标题:管理员:记录永久删除
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> AdminFlyProjectTimeaxisTitleDelete($fpAdminId)
     * 创建时间：2020-10-05 09:29:27
     * */
    public function AdminFlyProjectTimeaxisTitleDelete($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：获取数据表名称
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> GetTableName()
     * 创建时间：2020-10-05 09:29:27
     * */
    ...
    /**
     * 函数名称：获取类描述
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> GetClassDescript()
     * 创建时间：2020-10-05 09:29:27
     * */
    ...
    /**
     * 函数名称：获取数据表字段
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> GetTableField()
     * 创建时间：2020-10-05 09:29:27
     * */
    ...
    /**
     * 函数名称：类数据表基础字段检测
     * 函数调用：ObjFlyProjectTimeaxisTitle() -> OprationTableFieldBaseCheck()
     * 创建时间：2020-10-05 09:29:27
     * */
