...
/**------------------------------------*
 * 作者：shark
 * 创建时间：2020-10-20 11:28:10
 * Fly编码：1603164490596FLY879619
 * 类对象名：ObjFlyLogOnlykey()
 * ------------------------------------ */

//引入区

class FlyClassLogOnlykey{
    
    
    //---------- 类成员（member） ----------
    
    //类描述
    public static $classDescript = "OnlyKey记录";
    //类数据表名
    public static $tableName = "fly_log_onlykey";
    ...
    /**
     * 函数名称：OnlyKey记录:游客:记录查询
     * 函数调用：ObjFlyLogOnlykey() -> VisitorFlyLogOnlykeyPaging()
     * 创建时间：2020-10-20 11:28:10
     * */
    public function VisitorFlyLogOnlykeyPaging(){
        ...
        //$vDataField = "id,onlyKey,descript,indexNumber,updateTime,addTime,shelfState,state,recodeGroup,onlyKeyRecode,userName";
    ...
    /**
     * 函数名称：OnlyKey记录:系统:记录添加
     * 函数调用：ObjFlyLogOnlykey() -> SystemFlyLogOnlykeyAdd
     * 创建时间：2020-10-20 11:28:10
     * */
    public function SystemFlyLogOnlykeyAdd($fpRecodeGroup,$fpOnlyKeyRecode,$fpUserName){
        
        //--- Json组合区 ---
        //Json数组
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "recodeGroup,onlyKeyRecode,userName",
            "recodegroup" => $fpRecodeGroup,
            "onlykeyrecode" => $fpOnlyKeyRecode,
            "username" => $fpUserName,
            //"key_field" => "",
        );
        //执行:添加
    ...
    /**
     * 函数名称：OnlyKey记录:用户:记录查询
     * 函数调用：ObjFlyLogOnlykey() -> UserFlyLogOnlykeyPaging($fpUserId)
     * 创建时间：2020-10-20 11:28:10
     * */
    public function UserFlyLogOnlykeyPaging($fpUserId){
    ...
    /**
     * 函数名称：OnlyKey记录:管理员:记录添加
     * 函数调用：ObjFlyLogOnlykey() -> AdminFlyLogOnlykeyAdd($fpAdminId)
     * 创建时间：2020-10-20 11:28:10
     * */
    public function AdminFlyLogOnlykeyAdd($fpAdminId){
        ...
        //参数:onlyKey记录:onlyKeyRecode
        $pOnlyKeyRecode = GetParameterNoCode("onlykeyrecode",$json);
        if(!JudgeRegularFont($pOnlyKeyRecode)){return JsonModelParameterException("onlykeyrecode", $pOnlyKeyRecode, 36, "内容格式错误", __LINE__);}
        
        //参数:登录用户名:userName
        $pUserName = GetParameterNoCode("username",$json);
        if(!JudgeRegularFont($pUserName)){return JsonModelParameterException("username", $pUserName, 36, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "onlyKeyRecode", $pOnlyKeyRecode);
    ...
    /**
     * 函数名称：OnlyKey记录:管理员:记录查询
     * 函数调用：ObjFlyLogOnlykey() -> AdminFlyLogOnlykeyPaging($fpAdminId)
     * 创建时间：2020-10-20 11:28:10
     * */
    public function AdminFlyLogOnlykeyPaging($fpAdminId){
    ...
    /**
     * 函数名称：OnlyKey记录:管理员:记录修改
     * 函数调用：ObjFlyLogOnlykey() -> AdminFlyLogOnlykeySet($fpAdminId)
     * 创建时间：2020-10-20 11:28:10
     * */
    public function AdminFlyLogOnlykeySet($fpAdminId){
        ...
        //参数:onlyKey记录:onlyKeyRecode
        $pOnlyKeyRecode = GetParameterNoCode("onlykeyrecode",$json);
        if(!IsNull($pOnlyKeyRecode)&&!JudgeRegularFont($pOnlyKeyRecode)){return JsonModelParameterException("onlykeyrecode", $pOnlyKeyRecode, 36, "内容格式错误", __LINE__);}
        ...
        //判断字段值是否为空
        $vUpdateFieldJudgeInfor = JudgeFlyJsonAddUpdateField($jsonKeyValueArray,"descript,indexnumber,shelfstate,state,recodegroup,onlykeyrecode,username");
    ...
    /**
     * 函数名称：OnlyKey记录:管理员:记录状态修改
     * 函数调用：ObjFlyLogOnlykey() -> AdminFlyLogOnlykeySetState($fpAdminId)
     * 创建时间：2020-10-20 11:28:10
     * */
    public function AdminFlyLogOnlykeySetState($fpAdminId){
    ...
    /**
     * 函数名称：OnlyKey记录:管理员:数据上下架状态修改
     * 函数调用：ObjFlyLogOnlykey() -> AdminFlyLogOnlykeyShelfState($fpAdminId)
     * 创建时间：2020-10-20 11:28:10
     * */
    public function AdminFlyLogOnlykeyShelfState($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：OnlyKey记录:管理员:记录永久删除
     * 函数调用：ObjFlyLogOnlykey() -> AdminFlyLogOnlykeyDelete($fpAdminId)
     * 创建时间：2020-10-20 11:28:10
     * */
    public function AdminFlyLogOnlykeyDelete($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：获取数据表名称
     * 函数调用：ObjFlyLogOnlykey() -> GetTableName()
     * 创建时间：2020-10-20 11:28:10
     * */
    ...
    /**
     * 函数名称：获取类描述
     * 函数调用：ObjFlyLogOnlykey() -> GetClassDescript()
     * 创建时间：2020-10-20 11:28:10
     * */
    ...
    /**
     * 函数名称：获取数据表字段
     * 函数调用：ObjFlyLogOnlykey() -> GetTableField()
     * 创建时间：2020-10-20 11:28:10
     * */
    ...
    /**
     * 函数名称：类数据表创建
     * 函数调用：ObjFlyLogOnlykey() -> OprationCreateTable()
     * 创建时间：2020-10-20 11:28:10
     * */
    public function OprationCreateTable(){
        $sql = "CREATE TABLE `fly_log_onlykey` (  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '表ID',  `onlyKey` varchar(36) DEFAULT NULL COMMENT '唯一Key',  `descript` varchar(36) DEFAULT NULL COMMENT '记录描述',  `indexNumber` int(11) DEFAULT '-1' COMMENT '序号',  `updateTime` timestamp NULL DEFAULT NULL COMMENT '修改时间',  `addTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '添加时间',  `shelfState` varchar(36) DEFAULT 'true' COMMENT '上下架状态',  `state` varchar(36) DEFAULT 'STATE_NORMAL' COMMENT '记录状态',  `recodeGroup` varchar(36) DEFAULT NULL COMMENT '记录组',  `onlyKeyRecode` varchar(36) DEFAULT NULL COMMENT 'onlyKey记录',  `userName` varchar(36) DEFAULT NULL COMMENT '登录用户名',  PRIMARY KEY (`id`) USING BTREE) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='OnlyKey记录'";
    ...
    /**
     * 函数名称：类数据表基础字段检测
     * 函数调用：ObjFlyLogOnlykey() -> OprationTableFieldBaseCheck()
     * 创建时间：2020-10-20 11:28:10
     * */
