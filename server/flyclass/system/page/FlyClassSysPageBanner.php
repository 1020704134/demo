...
/**------------------------------------*
 * 作者：shark
 * 创建时间：2020-10-13 15:55:18
 * Fly编码：1602575718560FLY257679
 * 类对象名：ObjSysPageBanner()
 * ------------------------------------ */

//引入区

class FlyClassSysPageBanner{
    
    
    //---------- 类成员（member） ----------
    
    //类描述
    public static $classDescript = "系统-页面-Banner";
    //类数据表名
    public static $tableName = "sys_page_banner";
    ...
    /**
     * 函数名称：系统-页面-Banner:游客:记录查询
     * 函数调用：ObjSysPageBanner() -> VisitorSysPageBannerPaging()
     * 创建时间：2020-10-13 15:55:18
     * */
    public function VisitorSysPageBannerPaging(){
        ...
        //$vDataField = "id,onlyKey,descript,indexNumber,updateTime,addTime,shelfState,state,bannerGroup,bannerUrl,bannerHref";
    ...
    /**
     * 函数名称：系统-页面-Banner:系统:记录添加
     * 函数调用：ObjSysPageBanner() -> SystemSysPageBannerAdd
     * 创建时间：2020-10-13 15:55:18
     * */
    public function SystemSysPageBannerAdd($fpBannerGroup,$fpBannerUrl,$fpBannerHref){
        
        //--- Json组合区 ---
        //Json数组
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "bannerGroup,bannerUrl,bannerHref",
            "bannergroup" => $fpBannerGroup,
            "bannerurl" => $fpBannerUrl,
            "bannerhref" => $fpBannerHref,
            //"key_field" => "",
        );
        //执行:添加
    ...
    /**
     * 函数名称：系统-页面-Banner:用户:记录查询
     * 函数调用：ObjSysPageBanner() -> UserSysPageBannerPaging($fpUserId)
     * 创建时间：2020-10-13 15:55:18
     * */
    public function UserSysPageBannerPaging($fpUserId){
    ...
    /**
     * 函数名称：系统-页面-Banner:管理员:记录添加
     * 函数调用：ObjSysPageBanner() -> AdminSysPageBannerAdd($fpAdminId)
     * 创建时间：2020-10-13 15:55:18
     * */
    public function AdminSysPageBannerAdd($fpAdminId){
        ...
        //参数:Banner组:bannerGroup
        $pBannerGroup = GetParameterNoCode("bannergroup",$json);
        if(!JudgeRegularUrl($pBannerGroup)){return JsonModelParameterException("bannergroup", $pBannerGroup, 36, "URL地址格式错误", __LINE__);}
        
        //参数:Banner图片地址:bannerUrl
        $pBannerUrl = GetParameterNoCode("bannerurl",$json);
        if(!JudgeRegularUrl($pBannerUrl)){return JsonModelParameterException("bannerurl", $pBannerUrl, 128, "URL地址格式错误", __LINE__);}
        
        //参数:Banner链接:bannerHref
        $pBannerHref = GetParameterNoCode("bannerhref",$json);
        if(!JudgeRegularUrl($pBannerHref)){return JsonModelParameterException("bannerhref", $pBannerHref, 128, "URL地址格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "bannerGroup,bannerUrl,bannerHref",
            "bannergroup" => $pBannerGroup,
            "bannerurl" => $pBannerUrl,
            "bannerhref" => $pBannerHref,
        );
        //执行:添加
        $vJsonResult = MIndexDataInsert(JsonHandleArray($jsonKeyValueArray));
    ...
    /**
     * 函数名称：系统-页面-Banner:管理员:记录查询
     * 函数调用：ObjSysPageBanner() -> AdminSysPageBannerPaging($fpAdminId)
     * 创建时间：2020-10-13 15:55:18
     * */
    public function AdminSysPageBannerPaging($fpAdminId){
    ...
    /**
     * 函数名称：系统-页面-Banner:管理员:记录修改
     * 函数调用：ObjSysPageBanner() -> AdminSysPageBannerSet($fpAdminId)
     * 创建时间：2020-10-13 15:55:18
     * */
    public function AdminSysPageBannerSet($fpAdminId){
        ...
        //参数:Banner组:bannerGroup
        $pBannerGroup = GetParameterNoCode("bannergroup",$json);
        if(!IsNull($pBannerGroup)&&!JudgeRegularUrl($pBannerGroup)){return JsonModelParameterException("bannergroup", $pBannerGroup, 36, "URL地址格式错误", __LINE__);}
        
        //参数:Banner图片地址:bannerUrl
        $pBannerUrl = GetParameterNoCode("bannerurl",$json);
        if(!IsNull($pBannerUrl)&&!JudgeRegularUrl($pBannerUrl)){return JsonModelParameterException("bannerurl", $pBannerUrl, 128, "URL地址格式错误", __LINE__);}
        
        //参数:Banner链接:bannerHref
        $pBannerHref = GetParameterNoCode("bannerhref",$json);
        if(!IsNull($pBannerHref)&&!JudgeRegularUrl($pBannerHref)){return JsonModelParameterException("bannerhref", $pBannerHref, 128, "URL地址格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "bannerGroup", $pBannerGroup);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "bannerUrl", $pBannerUrl);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "bannerHref", $pBannerHref);
        
        //判断字段值是否为空
        $vUpdateFieldJudgeInfor = JudgeFlyJsonAddUpdateField($jsonKeyValueArray,"descript,indexnumber,shelfstate,state,bannergroup,bannerurl,bannerhref");
    ...
    /**
     * 函数名称：系统-页面-Banner:管理员:记录状态修改
     * 函数调用：ObjSysPageBanner() -> AdminSysPageBannerSetState($fpAdminId)
     * 创建时间：2020-10-13 15:55:18
     * */
    public function AdminSysPageBannerSetState($fpAdminId){
    ...
    /**
     * 函数名称：系统-页面-Banner:管理员:数据上下架状态修改
     * 函数调用：ObjSysPageBanner() -> AdminSysPageBannerShelfState($fpAdminId)
     * 创建时间：2020-10-13 15:55:18
     * */
    public function AdminSysPageBannerShelfState($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：系统-页面-Banner:管理员:记录永久删除
     * 函数调用：ObjSysPageBanner() -> AdminSysPageBannerDelete($fpAdminId)
     * 创建时间：2020-10-13 15:55:18
     * */
    public function AdminSysPageBannerDelete($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：获取数据表名称
     * 函数调用：ObjSysPageBanner() -> GetTableName()
     * 创建时间：2020-10-13 15:55:18
     * */
    ...
    /**
     * 函数名称：获取类描述
     * 函数调用：ObjSysPageBanner() -> GetClassDescript()
     * 创建时间：2020-10-13 15:55:18
     * */
    ...
    /**
     * 函数名称：获取数据表字段
     * 函数调用：ObjSysPageBanner() -> GetTableField()
     * 创建时间：2020-10-13 15:55:18
     * */
    ...
    /**
     * 函数名称：类数据表基础字段检测
     * 函数调用：ObjSysPageBanner() -> OprationTableFieldBaseCheck()
     * 创建时间：2020-10-13 15:55:18
     * */
