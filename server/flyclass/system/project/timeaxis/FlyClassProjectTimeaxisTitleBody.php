...
/**------------------------------------*
 * 作者：shark
 * 创建时间：2020-10-05 09:30:12
 * Fly编码：1601861412127FLY187234
 * 类对象名：ObjFlyProjectTimeaxisTitleBody()
 * ------------------------------------ */

//引入区

class FlyClassProjectTimeaxisTitleBody{
    
    
    //---------- 类成员（member） ----------
    
    //类描述
    public static $classDescript = "项目时间轴标题内容";
    //类数据表名
    public static $tableName = "fly_project_timeaxis_title_body";
    ...
    /**
     * 函数名称：项目时间轴标题内容:游客:记录查询
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> VisitorFlyProjectTimeaxisTitleBodyPaging()
     * 创建时间：2020-10-05 09:30:12
     * */
    public function VisitorFlyProjectTimeaxisTitleBodyPaging(){
        ...
        //$vDataField = "id,onlyKey,descript,indexNumber,updateTime,addTime,shelfState,state,versionTitleId,versionBodyType,versionBody";
    ...
    /**
     * 函数名称：项目时间轴标题内容:系统:记录添加
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> SystemFlyProjectTimeaxisTitleBodyAdd
     * 创建时间：2020-10-05 09:30:11
     * */
    public function SystemFlyProjectTimeaxisTitleBodyAdd($fpVersionTitleId,$fpVersionBodyType,$fpVersionBody){
        
        //--- Json组合区 ---
        //Json数组
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "descript,shelfState,versionTitleId,versionBodyType,versionBody",
            "descript" => self::$classDescript,
            "shelfstate" => "false",
            "versiontitleid" => $fpVersionTitleId,
            "versionbodytype" => $fpVersionBodyType,
            "versionbody" => $fpVersionBody,
            "key_field" => "versionTitleId",
        );
        //执行:添加
    ...
    /**
     * 函数名称：项目时间轴标题内容:用户:记录查询
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> UserFlyProjectTimeaxisTitleBodyPaging($fpUserId)
     * 创建时间：2020-10-05 09:30:12
     * */
    public function UserFlyProjectTimeaxisTitleBodyPaging($fpUserId){
    ...
    /**
     * 函数名称：项目时间轴标题内容:管理员:记录添加
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> AdminFlyProjectTimeaxisTitleBodyAdd($fpAdminId)
     * 创建时间：2020-10-05 09:30:11
     * */
    public function AdminFlyProjectTimeaxisTitleBodyAdd($fpAdminId){
        ...
        //参数:版本标题ID:versionTitleId
        $pVersionTitleId = GetParameterNoCode("versiontitleid",$json);
        if(!JudgeRegularFont($pVersionTitleId)){return JsonModelParameterException("versiontitleid", $pVersionTitleId, 20, "内容格式错误", __LINE__);}
        
        //参数:版本内容类型:versionBodyType
        $pVersionBodyType = GetParameterNoCode("versionbodytype",$json);
        if(!JudgeRegularFont($pVersionBodyType)){return JsonModelParameterException("versionbodytype", $pVersionBodyType, 36, "内容格式错误", __LINE__);}
        
        //参数:标题内容:versionBody
        $pVersionBody = GetParameterNoCode("versionbody",$json);
        if(!JudgeRegularFont($pVersionBody)){return JsonModelParameterException("versionbody", $pVersionBody, 128, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = array(
            ...
            "versiontitleid" => $pVersionTitleId,
            "versionbodytype" => $pVersionBodyType,
            "versionbody" => $pVersionBody,
            "key_field" => "versionTitleId,versionBodyType,versionBody",
        );
        //执行:添加
        $vJsonResult = MIndexDataInsert(JsonHandleArray($jsonKeyValueArray));
    ...
    /**
     * 函数名称：项目时间轴标题内容:管理员:记录查询
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> AdminFlyProjectTimeaxisTitleBodyPaging($fpAdminId)
     * 创建时间：2020-10-05 09:30:12
     * */
    public function AdminFlyProjectTimeaxisTitleBodyPaging($fpAdminId){
        ...
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "data_field" => $vDataField,
            "where_field" => "versionTitleId",
            "where_value" => "{$pVersionTitleId}",
            "page" => $pPage,
            "limit" => $pLimit,
            //"descbo" => "true",
            //"orderby" => "id",
    ...
    /**
     * 函数名称：项目时间轴标题内容:管理员:记录修改
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> AdminFlyProjectTimeaxisTitleBodySet($fpAdminId)
     * 创建时间：2020-10-05 09:30:11
     * */
    public function AdminFlyProjectTimeaxisTitleBodySet($fpAdminId){
        ...
        //参数:版本标题ID:versionTitleId
        $pVersionTitleId = GetParameterNoCode("versiontitleid",$json);
        if(!IsNull($pVersionTitleId)&&!JudgeRegularFont($pVersionTitleId)){return JsonModelParameterException("versiontitleid", $pVersionTitleId, 20, "内容格式错误", __LINE__);}
        
        //参数:版本内容类型:versionBodyType
        $pVersionBodyType = GetParameterNoCode("versionbodytype",$json);
        if(!IsNull($pVersionBodyType)&&!JudgeRegularFont($pVersionBodyType)){return JsonModelParameterException("versionbodytype", $pVersionBodyType, 36, "内容格式错误", __LINE__);}
        
        //参数:标题内容:versionBody
        $pVersionBody = GetParameterNoCode("versionbody",$json);
        if(!IsNull($pVersionBody)&&!JudgeRegularFont($pVersionBody)){return JsonModelParameterException("versionbody", $pVersionBody, 128, "内容格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "versionTitleId", $pVersionTitleId);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "versionBodyType", $pVersionBodyType);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "versionBody", $pVersionBody);
        
        //判断字段值是否为空
        $vUpdateFieldJudgeInfor = JudgeFlyJsonAddUpdateField($jsonKeyValueArray,"descript,indexnumber,shelfstate,state,versiontitleid,versionbodytype,versionbody");
    ...
    /**
     * 函数名称：项目时间轴标题内容:管理员:记录状态修改
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> AdminFlyProjectTimeaxisTitleBodySetState($fpAdminId)
     * 创建时间：2020-10-05 09:30:12
     * */
    public function AdminFlyProjectTimeaxisTitleBodySetState($fpAdminId){
    ...
    /**
     * 函数名称：项目时间轴标题内容:管理员:数据上下架状态修改
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> AdminFlyProjectTimeaxisTitleBodyShelfState($fpAdminId)
     * 创建时间：2020-10-05 09:30:12
     * */
    public function AdminFlyProjectTimeaxisTitleBodyShelfState($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：项目时间轴标题内容:管理员:记录永久删除
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> AdminFlyProjectTimeaxisTitleBodyDelete($fpAdminId)
     * 创建时间：2020-10-05 09:30:12
     * */
    public function AdminFlyProjectTimeaxisTitleBodyDelete($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：获取数据表名称
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> GetTableName()
     * 创建时间：2020-10-05 09:30:11
     * */
    ...
    /**
     * 函数名称：获取类描述
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> GetClassDescript()
     * 创建时间：2020-10-05 09:30:11
     * */
    ...
    /**
     * 函数名称：获取数据表字段
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> GetTableField()
     * 创建时间：2020-10-05 09:30:11
     * */
    ...
    /**
     * 函数名称：类数据表创建
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> OprationCreateTable()
     * 创建时间：2020-10-05 09:30:11
     * */
    public function OprationCreateTable(){
        $sql = "CREATE TABLE `fly_project_timeaxis_title_body` (  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '表ID',  `onlyKey` varchar(36) DEFAULT NULL COMMENT '唯一Key',  `descript` varchar(36) DEFAULT NULL COMMENT '记录描述',  `indexNumber` int(11) DEFAULT '-1' COMMENT '序号',  `updateTime` timestamp NULL DEFAULT NULL COMMENT '修改时间',  `addTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '添加时间',  `shelfState` varchar(36) DEFAULT 'true' COMMENT '上下架状态',  `state` varchar(36) DEFAULT 'STATE_NORMAL' COMMENT '记录状态',  `versionTitleId` varchar(20) DEFAULT NULL COMMENT '版本标题ID',  `versionBodyType` varchar(36) DEFAULT NULL COMMENT '版本内容类型',  `versionBody` varchar(128) DEFAULT NULL COMMENT '标题内容',  PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='项目时间轴标题内容'";
    ...
    /**
     * 函数名称：类数据表基础字段检测
     * 函数调用：ObjFlyProjectTimeaxisTitleBody() -> OprationTableFieldBaseCheck()
     * 创建时间：2020-10-05 09:30:11
     * */
