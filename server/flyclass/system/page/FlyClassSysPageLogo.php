...
/**------------------------------------*
 * 作者：shark
 * 创建时间：2020-10-13 08:44:47
 * Fly编码：1602549887374FLY648749
 * 类对象名：ObjSysPageLogo()
 * ------------------------------------ */

//引入区

class FlyClassSysPageLogo{
    
    
    //---------- 类成员（member） ----------
    
    //类描述
    public static $classDescript = "系统-页面-LOGO";
    //类数据表名
    public static $tableName = "sys_page_logo";
    ...
    /**
     * 函数名称：系统-页面-LOGO:游客:记录查询
     * 函数调用：ObjSysPageLogo() -> VisitorSysPageLogoPaging()
     * 创建时间：2020-10-13 08:44:47
     * */
    public function VisitorSysPageLogoPaging(){
        ...
        //$vDataField = "id,onlyKey,descript,indexNumber,updateTime,addTime,shelfState,state,logoWidth,logoHeight,logoName,logoUrl";
    ...
    /**
     * 函数名称：系统-页面-LOGO:系统:记录添加
     * 函数调用：ObjSysPageLogo() -> SystemSysPageLogoAdd
     * 创建时间：2020-10-13 08:44:47
     * */
    public function SystemSysPageLogoAdd($fpLogoWidth,$fpLogoHeight,$fpLogoName,$fpLogoUrl){
        
        //--- Json组合区 ---
        //Json数组
        $jsonKeyValueArray = array(
            "table_name" => self::$tableName,
            "insert_field" => "descript,shelfState,logoWidth,logoHeight,logoName,logoUrl",
            "descript" => self::$classDescript,
            "shelfstate" => "false",
            "logowidth" => $fpLogoWidth,
            "logoheight" => $fpLogoHeight,
            "logoname" => $fpLogoName,
            "logourl" => $fpLogoUrl,
            //"key_field" => "",
        );
    ...
    /**
     * 函数名称：系统-页面-LOGO:用户:记录查询
     * 函数调用：ObjSysPageLogo() -> UserSysPageLogoPaging($fpUserId)
     * 创建时间：2020-10-13 08:44:47
     * */
    public function UserSysPageLogoPaging($fpUserId){
    ...
    /**
     * 函数名称：系统-页面-LOGO:管理员:记录添加
     * 函数调用：ObjSysPageLogo() -> AdminSysPageLogoAdd($fpAdminId)
     * 创建时间：2020-10-13 08:44:47
     * */
    public function AdminSysPageLogoAdd($fpAdminId){
        ...
        //参数:LOGO宽:logoWidth
        $pLogoWidth = GetParameterNoCode("logowidth",$json);
        if(!JudgeRegularUrl($pLogoWidth)){return JsonModelParameterException("logowidth", $pLogoWidth, 10, "URL地址格式错误", __LINE__);}
        
        //参数:LOGO高:logoHeight
        $pLogoHeight = GetParameterNoCode("logoheight",$json);
        if(!JudgeRegularUrl($pLogoHeight)){return JsonModelParameterException("logoheight", $pLogoHeight, 10, "URL地址格式错误", __LINE__);}
        
        //参数:LOGO名称:logoName
        $pLogoName = "W{$pLogoWidth}H{$pLogoHeight}";
        
        //参数:LOGO图片地址:logoUrl
        $pLogoUrl = GetParameterNoCode("logourl",$json);
        if(!JudgeRegularUrl($pLogoUrl)){return JsonModelParameterException("logourl", $pLogoUrl, 128, "URL地址格式错误", __LINE__);}
    ...
    /**
     * 函数名称：系统-页面-LOGO:管理员:记录查询
     * 函数调用：ObjSysPageLogo() -> AdminSysPageLogoPaging($fpAdminId)
     * 创建时间：2020-10-13 08:44:47
     * */
    public function AdminSysPageLogoPaging($fpAdminId){
    ...
    /**
     * 函数名称：系统-页面-LOGO:管理员:记录修改
     * 函数调用：ObjSysPageLogo() -> AdminSysPageLogoSet($fpAdminId)
     * 创建时间：2020-10-13 08:44:47
     * */
    public function AdminSysPageLogoSet($fpAdminId){
        ...
        //参数:LOGO宽:logoWidth
        $pLogoWidth = GetParameterNoCode("logowidth",$json);
        if(!IsNull($pLogoWidth)&&!JudgeRegularUrl($pLogoWidth)){return JsonModelParameterException("logowidth", $pLogoWidth, 10, "URL地址格式错误", __LINE__);}
        
        //参数:LOGO高:logoHeight
        $pLogoHeight = GetParameterNoCode("logoheight",$json);
        if(!IsNull($pLogoHeight)&&!JudgeRegularUrl($pLogoHeight)){return JsonModelParameterException("logoheight", $pLogoHeight, 10, "URL地址格式错误", __LINE__);}
        
        //参数:LOGO名称:logoName
        $pLogoName = GetParameterNoCode("logoname",$json);
        if(!IsNull($pLogoName)&&!JudgeRegularUrl($pLogoName)){return JsonModelParameterException("logoname", $pLogoName, 36, "URL地址格式错误", __LINE__);}
        
        //参数:LOGO图片地址:logoUrl
        $pLogoUrl = GetParameterNoCode("logourl",$json);
        if(!IsNull($pLogoUrl)&&!JudgeRegularUrl($pLogoUrl)){return JsonModelParameterException("logourl", $pLogoUrl, 128, "URL地址格式错误", __LINE__);}
        ...
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "logoWidth", $pLogoWidth);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "logoHeight", $pLogoHeight);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "logoName", $pLogoName);
        $jsonKeyValueArray = HandleFlyJsonAddUpdateField($jsonKeyValueArray, "logoUrl", $pLogoUrl);
        
        //判断字段值是否为空
        $vUpdateFieldJudgeInfor = JudgeFlyJsonAddUpdateField($jsonKeyValueArray,"descript,indexnumber,shelfstate,state,logowidth,logoheight,logoname,logourl");
    ...
    /**
     * 函数名称：系统-页面-LOGO:管理员:记录状态修改
     * 函数调用：ObjSysPageLogo() -> AdminSysPageLogoSetState($fpAdminId)
     * 创建时间：2020-10-13 08:44:47
     * */
    public function AdminSysPageLogoSetState($fpAdminId){
    ...
    /**
     * 函数名称：系统-页面-LOGO:管理员:数据上下架状态修改
     * 函数调用：ObjSysPageLogo() -> AdminSysPageLogoShelfState($fpAdminId)
     * 创建时间：2020-10-13 08:44:47
     * */
    public function AdminSysPageLogoShelfState($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：系统-页面-LOGO:管理员:记录永久删除
     * 函数调用：ObjSysPageLogo() -> AdminSysPageLogoDelete($fpAdminId)
     * 创建时间：2020-10-13 08:44:47
     * */
    public function AdminSysPageLogoDelete($fpAdminId){
        //--- 参数限制区 ---
        $vParameterArray = GetArray(FLY_PARAMETER_LIMIT, ",");
        $bParameterJudge = JudgeParameterLimit($vParameterArray);
        if(JudgeJsonFalse($bParameterJudge)){return $bParameterJudge;}
    ...
    /**
     * 函数名称：获取数据表名称
     * 函数调用：ObjSysPageLogo() -> GetTableName()
     * 创建时间：2020-10-13 08:44:47
     * */
    ...
    /**
     * 函数名称：获取类描述
     * 函数调用：ObjSysPageLogo() -> GetClassDescript()
     * 创建时间：2020-10-13 08:44:47
     * */
    ...
    /**
     * 函数名称：获取数据表字段
     * 函数调用：ObjSysPageLogo() -> GetTableField()
     * 创建时间：2020-10-13 08:44:47
     * */
    ...
    /**
     * 函数名称：类数据表基础字段检测
     * 函数调用：ObjSysPageLogo() -> OprationTableFieldBaseCheck()
     * 创建时间：2020-10-13 08:44:47
     * */
